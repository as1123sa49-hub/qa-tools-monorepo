...
- 產出：`data/{userId}/captures/{env}/{lang}/{SlotId}/`（多人工作區隔離，見下方）
...
同一台機器跑 `:3847` 時，不同 QA 可用 **使用者 ID** 隔離登入狀態、截圖與 OCR 快取：

| 項目 | 路徑 |
|------|------|
| 工作區根目錄 | `data/{userId}/` |
| 登入狀態 | `data/{userId}/.auth/lobby.json` |
| 擷取產出 | `data/{userId}/captures/{env}/{lang}/{SlotId}/` |

- UI 頂部「使用者 ID」欄位（預設 `default`，存於 sessionStorage）
- API / CLI：`X-User-Id` Header、body `userId`、或 query `?userId=`（擷圖預覽）
- 同一 userId 重複發起擷取/驗證會 **中止上一個任務**；不同 userId 可並行（Playwright 視窗仍在跑 server 的本機）

**從舊版遷移**（可選）：將根目錄 `.auth/`、`captures/` 搬到 `data/default/` 即可沿用既有資料。

```powershell
# 為 alice 儲存獨立登入
node capture-cli.js --save-auth --user alice

# alice 的工作區擷取
node capture-cli.js --user alice --env uat --lang bn --slot Slot015 --xlsx "..."
```
...
在開啟的瀏覽器手動登入大廳，回到終端機按 Enter，會寫入 `data/{userId}/.auth/lobby.json`（勿提交 git）。
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/api/status` | 是否在擷取 / 驗證（query `userId` 或 Header `X-User-Id`） |
...
所有 POST 與擷圖 GET 支援 **`X-User-Id`** Header 或 body/query **`userId`**（缺省 `default`）。

程式對外入口：`lib/capture-flow.js`（re-export `runSlotCapture`、`loadConfig` 等）。工作區路徑見 `lib/user-context.js`。
...
`data/`、`.auth/`、`captures/`、`config.json` 已在 `.gitignore`；本機設定與擷取產出請勿 push。
